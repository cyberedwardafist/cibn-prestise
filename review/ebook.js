...
const EbookLib = {
    ...
    async load() {
        ...
        if (grid) grid.innerHTML = '<div class="empty-state"><p>Memuat modul...</p></div>';
        try {
            ...
            if (grid) grid.innerHTML = '<div class="empty-state"><p>Gagal memuat library</p><small>' + _ebkEsc(e.message || '') + '</small></div>';
        }
    },
    ...
    _renderModulDetail(grid) {
        const m = this.modules.find(x => x.kode === this._openModulKode);
        if (!m) {
            grid.innerHTML = '<div class="empty-state"><p>Modul tidak ditemukan</p></div>';
            return;
        }
        ...
        if (!books.length) {
            grid.innerHTML = q
                ? '<div class="empty-state"><p>Tidak ditemukan</p><small>Coba kata kunci lain</small></div>'
                : '<div class="empty-state"><p>Belum ada buku di modul ini</p></div>';
            return;
        }
