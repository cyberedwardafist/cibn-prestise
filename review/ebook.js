...
const EbookLib = {
    ...
    async load() {
        ...
        if (grid) grid.innerHTML = '<div class="empty-state"><p>Memuat buku...</p></div>';
        try {
    ...
    _render() {
        ...
        if (!data.length) {
            if (this.books.length && (q || this._kelompokFilter !== 'all')) {
                ...
                grid.innerHTML = '<div class="empty-state"><p>Belum ada buku</p><small>Buku yang ditambahkan admin akan muncul di sini</small></div>';
            }
            return;
        }
