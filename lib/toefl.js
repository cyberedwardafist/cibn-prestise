/* =============================================================================
...

   {
     listening: {
       ...
       soal: [
         { id, audio_url, pertanyaan, jawaban:[{id,teks}], kunci:['A'] }
       ]
     },
     structure: {
       soal: [
 ...
 *  (soal 1 bacaan selalu berdekatan), tapi urutan ANTAR blok bacaan ikut
 *  diacak (peserta lain bisa dapat urutan bacaan berbeda), dan urutan blok
...
function shuffleReadingSoal(items) {
    ...
    const out = [];
    for (const key of shuffledGroupOrder) out.push(...groups.get(key)); // urutan DALAM 1 bacaan tetap (soal 1,2,3 dst)
    return out;
}
