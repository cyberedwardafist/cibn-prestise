...
async function submitBukuForm() {
    ...
    try {
        const payload = { nama, kelompok };

        // File PDF & poster (kalau dipilih baru) di-upload LANGSUNG dari browser ke
        // Supabase Storage lewat signed URL SEBELUM metadata dikirim ke server kita —
        // server.js sendiri tidak pernah menerima isi filenya.
        if (EbookFormState.pdfFile) {
            if (btn) btn.textContent = 'Mengupload PDF...';
            const pdfRes = await apiUploadEbookFile('ebook-pdf', EbookFormState.pdfFile, nama);
            if (!pdfRes || pdfRes.error || pdfRes.rejected || pdfRes.networkError) {
                showToast('Gagal upload PDF: ' + (pdfRes?.error || 'tidak bisa terhubung ke storage'), 'danger');
                return;
            }
            const jumlahHalaman = await countPdfPagesClient(EbookFormState.pdfFile);
            payload.pdf = { url: pdfRes.url, originalName: EbookFormState.pdfFile.name, size: EbookFormState.pdfFile.size, pages: jumlahHalaman };
        }
        if (EbookFormState.posterFile) {
            if (btn) btn.textContent = 'Mengupload poster...';
            const posterRes = await apiUploadEbookFile('ebook-poster', EbookFormState.posterFile, nama);
            if (!posterRes || posterRes.error || posterRes.rejected || posterRes.networkError) {
                showToast('Gagal upload poster: ' + (posterRes?.error || 'tidak bisa terhubung ke storage'), 'danger');
                return;
            }
            payload.poster = { url: posterRes.url };
        }

        if (btn) btn.textContent = 'Menyimpan...';
        const result = isEdit ? await EbookAPI.update(EbookFormState.kode, payload) : await EbookAPI.create(payload);
...
async function submitEbookModulForm() {
    ...
    try {
        const payload = { nama, kelompok, ebook_list };

        // Poster (kalau dipilih baru) di-upload LANGSUNG ke Supabase Storage lewat
        // signed URL sebelum metadata dikirim — server tidak menerima isi filenya.
        if (EbookModulFormState.posterFile) {
            if (saveBtn) saveBtn.textContent = 'Mengupload poster...';
            const posterRes = await apiUploadEbookFile('ebook-modul-poster', EbookModulFormState.posterFile, nama);
            if (!posterRes || posterRes.error || posterRes.rejected || posterRes.networkError) {
                showToast('Gagal upload poster: ' + (posterRes?.error || 'tidak bisa terhubung ke storage'), 'danger');
                return;
            }
        ...
        if (mode === 'add') await EbookModulAPI.create(payload);
        else await EbookModulAPI.update(kode, payload);
