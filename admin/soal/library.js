...
const _libTypeOptions=[{value:'all',label:'Semua Tipe'},{value:'multiple_choice',label:'Multiple Choice'},{value:'linier',label:'Linier'},{value:'sikap_kerja',label:'Sikap Kerja'},{value:'toefl',label:'TOEFL ITP'}];
...
function _libSwipeCardHtml(s){const kode=s.kode||s.id;const sel=_libSelected.has(kode);return SwipeCards.buildSwipeCardHtml({
    ...
    sub:soalTypeLabel(s.type)+(_soalKelompokNama(s.kelompok)?' · '+_soalKelompokNama(s.kelompok):'')+' · '+formatDate(s.created_at),
    leftActions:[
...
function _renderLibList(){
    let data=_libData;
    if(_libSearch)data=data.filter(s=>(s.nama||'').toLowerCase().includes(_libSearch.toLowerCase())||(s.nama_internal||'').toLowerCase().includes(_libSearch.toLowerCase())||(s.type||'').toLowerCase().includes(_libSearch.toLowerCase())||(_soalKelompokNama(s.kelompok)||'').toLowerCase().includes(_libSearch.toLowerCase()));
